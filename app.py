...
with st.sidebar:
    ...
    parquet_path = st.text_input("Parquet path", value=str(DEFAULT_PARQUET))
...
# =============================
    ...
    if st.button("Run regression"):
        try:
            ...
            # ---- Rename exposure label in results table for GUI/export ----
            pretty_x = exposure_label  # or use x_col if you want just the raw column name

            if isinstance(res, pd.DataFrame):
                # Common patterns: "Variable" column OR variable name as index
                if "Variable" in res.columns:
                    res["Variable"] = res["Variable"].replace({"__x__": pretty_x})
                else:
                    res = res.rename(index={"__x__": pretty_x})

                # Also handle any sklearn/statsmodels-style prefixes if they appear
                res = res.replace({"__x__": pretty_x})
